...
export class Ground {
  ...
  createTechProps() {
    ...
    holos.forEach(([x,z]) => {
      ...
      const holo = new THREE.Mesh(new THREE.ConeGeometry(0.8,2,6), new THREE.MeshStandardMaterial({color:0x00ffff,emissive:0x00ffff,emissiveIntensity:0.5,transparent:true,opaTOWN:0.5}));
  ...
  createPonds() {
    ...
    ponds.forEach(([x,z]) => {
      const g = new THREE.Group();
      const water = new THREE.Mesh(new THREE.CircleGeometry(2.5,16), new THREE.MeshStandardMaterial({color:0x4488bb,transparent:true,opaTOWN:0.7,roughness:0.1}));
  ...
  createEasterEggs() {
    ...
    const dome = new THREE.Mesh(new THREE.SphereGeometry(0.6,12,6,0,Math.PI*2,0,Math.PI/2), new THREE.MeshStandardMaterial({color:0x88ccff,transparent:true,opaTOWN:0.6}));
  ...
  createTOWNProps() {
    ...
    vending.forEach(([x,z],i) => {
      ...
      body.position.y = 1; body.castShadow = true; g.add(body);
      const glass = new THREE.Mesh(new THREE.BoxGeometry(0.7,1.2,0.05), new THREE.MeshStandardMaterial({color:0xaaddff,emissive:0xaaddff,emissiveIntensity:0.2,transparent:true,opaTOWN:0.7}));
    ...
    busStops.forEach(([x,z]) => {
      ...
      const glassMat = new THREE.MeshStandardMaterial({color:0x88ccff,transparent:true,opaTOWN:0.4});
      // Posts
      [-1,1].forEach(xOff => {
    ...
    stands.forEach(([x,z]) => {
      ...
      const glass = new THREE.Mesh(new THREE.BoxGeometry(0.6,0.6,0.05), new THREE.MeshStandardMaterial({color:0xffffff,transparent:true,opaTOWN:0.6}));
