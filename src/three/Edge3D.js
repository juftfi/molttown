...
export class Edge3D {
  ...
  create() {
    ...
    const opaTOWN = Math.min(0.3 + this.weight * 0.15, 0.8);
  ...
  update(time) {
    ...
    const baseOpaTOWN = Math.min(0.3 + this.weight * 0.15, 0.8);
    ...
    if (this.isPulsing) {
      ...
      if (progress >= 1) {
        ...
        this.material.opaTOWN = baseOpaTOWN;
      } else {
        // Pulse in progress - bright gold that fades
        const intensity = 1 - progress; // 1 -> 0
        this.material.opaTOWN = baseOpaTOWN + intensity * 0.5;
      ...
      this.material.opaTOWN = baseOpaTOWN + Math.sin(time * 0.002) * 0.1;
    }
  }
